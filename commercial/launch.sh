...
Usage: $(basename "$0") [OPTION]...
  -h          Display help
  -c VALUE    Collection names wrapped by double quote mini/mini_dual/open_images/illust/text/text_short/basic/wiki/wiki_org/text_large
  -d VALUE    Dowload data type: mini/basic/open_images/full
...
download_data() {
  ...
  if [ "$dlType" = "full" -o "$dlType" = "wiki" -o "$dlType" = "open_images" ]; then
    if [ ! -d $OPEN_IMAGES_DIR -a "$dlType" != "wiki" ]; then
        make open_images
    else
        echo "[INFO] $OPEN_IMAGES_DIR is already existing. If you want to download again, please remove $OPEN_IMAGES_DIR"
    fi
    if [ `find $WIKI_DIR -name "jamwiki-*-cirrussearch-content.json.gz" 2>&1 | wc -l` -eq 0 -a "$dlType" != "open_images" ]; then
